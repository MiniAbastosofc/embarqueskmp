...
@Composable
fun DescripcionIncidencias() {

    val incidenciasViewModel = koinViewModel<IncidenciasViewModel>()
    val state by incidenciasViewModel.state.collectAsState()

    var texto by remember(state.descripcionIncidencia)
    { mutableStateOf(state.descripcionIncidencia) }
    ...
    Column(
        ...
        OutlinedTextField(
            ...
            onValueChange = {
                texto = it
                incidenciasViewModel.descripcionIncidenciaTexto(it)
            },
            label = { Text("Describe brevemente lo que sucedió") },
